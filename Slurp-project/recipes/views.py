...
def home(request):
    ...
    page_title = 'Home'
    return render(request, "recipes/home.html", {'recipes':recipes, 'page_title':page_title})
...
def new_recipe(request):
    if request.method == 'POST':
        ...
        if filled_form.is_valid():
            ...
            message = "New recipe (%s) created successfully!" %(filled_form.cleaned_data['name'])
            filled_form = RecipeForm()
        else:
            message = "Update failed. Pelase try again."
            created_recipe_id = None
        return render(request, "recipes/new_recipe.html", {'created_recipe_id':created_recipe_id,'recipeform':filled_form, 'message':message})
    else:
        form = RecipeForm()
